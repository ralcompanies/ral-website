---
title: About
eyebrow: About RAL
headline: Founded as a design firm in 1979. Design is still in our DNA.
lede: Today RAL is also a developer, owner's representative and partner on complex residential, hospitality and public-private projects.
story:
  - heading: Designer.
    text: RAL designs office, residential, hospitality and resort properties in the United States and abroad. Because the company began as an architecture firm, its in-house architects and the design teams it hires work toward timeless, original buildings that respond to their surroundings.
  - heading: Developer.
    text: RAL has a reputation for completing projects on time and on budget. It brings an experienced, multidisciplinary team to complex and challenging projects, from site acquisition and financing through construction, sales and leasing.
  - heading: Partner.
    text: RAL works for and alongside equity investors, owners, lenders and operators, and it has a long record of partnering with governments and local communities to create new places and meet real economic development goals.
capabilities:
  - key: develop
    title: Development
    text: Acquisition, entitlement, financing, design management, construction and sales for RAL's own projects and those it co-develops with partners.
  - key: partner
    title: Partnership
    text: Co-development and joint ventures with capital partners, and competitive public proposals with agencies such as NYCEDC and Brooklyn Bridge Park Corporation.
  - key: manage
    title: Owner's representation
    text: Full-scale development management for owners, lenders and operators, as well as renovations, repositionings and completions, from planning and design through construction and opening.
  - key: design
    title: Design & architecture
    text: In-house architects and designers who plan buildings from the floor plan out and lead the design teams on every project.
  - key: property
    title: Property management & leasing
    text: Management and office and retail leasing for properties in RAL's own portfolio.
how_we_work: >-
  RAL applies a varied group of professionals to each project and combines their abilities into
  a team that is greater than the sum of its parts. The core management team has worked together
  for more than 20 years, and that shared experience gives partners a faster, more dependable
  development process.
internal_notes: ["Story and how-we-work copy adapted from the live ralcompanies.com About page. Capability copy is new; review wording."]
---
